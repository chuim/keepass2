...
namespace KeePass.Util
{
	...
	public static class EntryUtil
	{
			...
			SprContext ctx)
		{
			...
			const Char CharNewPwProfileDiv = ':';
			const string StrNewPwEnd = @"}";
			int nNPStart = str.IndexOf(StrNewPwStart, StrUtil.CaseIgnoreCmp);
			int nNPAfterStart = Math.Min(nNPStart + StrNewPwStart.Length, str.Length);
			int nNPEnd = str.IndexOf(StrNewPwEnd, nNPAfterStart, StrUtil.CaseIgnoreCmp);
			if(nNPStart > 0 && nNPEnd >= nNPAfterStart)
			{
				PwProfile pwProfile = null;
				if(nNPEnd == nNPAfterStart) // Not using a custom gen profile
				{
					pwProfile = Program.Config.PasswordGenerator.AutoGeneratedPasswordsProfile;
				}
				else if(str[nNPAfterStart] == CharNewPwProfileDiv) // Using a custom profile
				{
					string StrProfileName = str.Substring(nNPAfterStart + 1,
														  nNPEnd - (nNPAfterStart + 1));
					pwProfile = Program.Config.PasswordGenerator.UserProfiles.Find(
									p => p.Name == StrProfileName);
				}
				if(pwProfile != null)
				{
					ProtectedString psAutoGen;
					PwgError e = PwGenerator.Generate(out psAutoGen, pwProfile, 
					...
					if(e == PwgError.Success) {
						...
						int nOffset = 0;
						while(nOffset < str.Length)
						{
							int nStart = str.IndexOf(StrNewPwStart, nOffset, StrUtil.CaseIgnoreCmp);
							if (nStart < 0) break;
							int nEnd = str.IndexOf(StrNewPwEnd, nStart + 1, StrUtil.CaseIgnoreCmp);
							if (nEnd <= nStart) break;

							str = str.Remove(nStart, nEnd - nStart + 1);
							str = str.Insert(nStart, strIns);
							nOffset = nStart + strIns.Length;
						}
					}
				}
			}
